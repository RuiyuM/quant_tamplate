...
async def post_spot_order(exchange, symbol, side, type, quantity, timestamp, chat_id, bot, limit_price=None):
    max_retries = 5  # Maximum number of retries before giving up
    retries = 0
    ...
    while retries < max_retries:
        try:
            if type == "MARKET":
                martker_order_res = exchange.privatePostOrder({
                    "symbol": symbol,  # Symbol must be in the format like 'BTCUSDT'
                    "side": side,  # 'BUY' or 'SELL'
                    "type": type,
                    "quantity": quantity,
                ...
                order_id = int(martker_order_res['orderId'])
                # await send_text_message(bot=bot, msg="### Market Open Position Triggered Successfully! ###",
                #                         chat_id=chat_id)
                return order_id
            elif type == "LIMIT":
                limit_order_res = exchange.privatePostOrder({
                    "symbol": symbol,  # Symbol must be in the format like 'BTCUSDT'
                    "side": side,  # 'BUY' or 'SELL'
                    "type": type,
                    "quantity": quantity,
                ...
                order_id = int(limit_order_res['orderId'])
                return order_id
...
async def get_spot_order_status(exchange, order_id_str, futures_symbol):
    max_retries = 5  # Maximum number of retries before giving up
    retries = 0

    while retries < max_retries:
        try:
            status = exchange.fetch_order(order_id_str, futures_symbol)
            return status
            ...
            retries += 1
            await asyncio.sleep(0.1)  # Wait before retrying
            # _, timestamp = await sync_server_time(exchange)
...
async def get_spot_account_info(exchange, server_time):
    max_retries = 5  # Maximum number of retries before giving up
    retries = 0
